...
let eventsQuery = fs.readFileSync(
  path.join(__dirname, "../db/events/select_events.sql"),
  "utf-8"
);
...
let event_locations_query = fs.readFileSync(
  path.join(__dirname, "../db/events/select_event_locations.sql"),
  "utf-8"
);
let event_types_query = fs.readFileSync(
  path.join(__dirname, "../db/events/select_event_types.sql"),
  "utf-8"
);
...
let singleEventQuery = fs.readFileSync(
  path.join(__dirname, "../db/events/select_event_single.sql"),
  "utf-8"
);
...
let singleEventForFormQuery = fs.readFileSync(
  path.join(__dirname, "../db/events/select_event_single_form.sql"),
  "utf-8"
);
...
router.get("/:event_id/modify", async function (req, res, next) {
  try {
    ...
    res.render("eventform", {
      ...
      event_types: event_types,
      event: event_data,
    }); // provide current event data
  } catch (err) {
    next(err);
  }
});

let insertEventQuery = fs.readFileSync(
  path.join(__dirname, "../db/events/insert_event.sql"),
  "utf-8"
);
// (`event_name`, `event_location_id`, `event_type_id`, `event_dt`, `event_duration`, `event_description`)
router.post("/", async function (req, res, next) {
  try {
    let results = await db.queryPromise(insertEventQuery, [
      req.body.event_name,
...
let updateEventQuery = fs.readFileSync(
  path.join(__dirname, "../db/events/update_event.sql"),
  "utf-8"
);
router.post("/:event_id", async function (req, res, next) {
  try {
    let results = await db.queryPromise(updateEventQuery, [
      req.body.event_name,
      req.body.event_location_id,
      req.body.event_type_id,
      `${req.body.event_date} ${req.body.event_time}`,
